...
2. **Gather signal** (run in parallel):
   ...
   - `gh issue list --assignee @me --state open --json number,title,labels,url` — assigned issues; the ones tagged `claude-implement` are M5 hero candidates.
3. **Classify.**
   - **Yesterday:** every commit in the window, collapsed to one bullet per scope (e.g. three `M4(evals): …` commits become one `finished M4 evals harness` bullet).
   - **Today:** derived from (a) branch name + working-tree state, (b) the next bullet in the current milestone from [PLAN.md](PLAN.md) if the working tree is clean, (c) any PR with `reviewDecision: CHANGES_REQUESTED`. If none of these apply, print "TBD — pick from PLAN.md" rather than inventing a task.
...
- **Don't invent "Today" items.** If the working tree is clean, branch is `main`, no PRs are in flight, and no issues are assigned, print "TBD — pick from [PLAN.md](PLAN.md)" and leave the call to the author. Inventing a plausible-sounding next task is worse than no task.
